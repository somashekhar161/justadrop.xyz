...
interface InProgressPageProps {
  title: string;
  description?: string;
  /** When true, renders only the content (no header/footer) - for use inside AppLayout */
  embedded?: boolean;
}

export function InProgressPage({ title, description, embedded }: InProgressPageProps) {
  const content = (
    <div className="flex flex-1 items-center justify-center px-4 py-20">
      <div className="mx-auto max-w-md text-center">
        ...
        >
          ...
          {embedded ? 'Back to dashboard' : 'Back to home'}
        </Link>
      </div>
    </div>
  );
